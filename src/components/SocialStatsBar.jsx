...
export default function SocialStatsBar() {
  return (
    <motion.div
      ...
      className="flex flex-wrap justify-center gap-4 py-10 px-6"
    >
      {stats.map((stat, i) => (
        <motion.a
          key={stat.platform}
          ...
          className={`flex items-center gap-3 px-6 py-4 rounded-2xl border ${stat.borderColor} ${stat.bgColor} hover:scale-105 transition-transform duration-200 cursor-pointer`}
        >
          <span className={stat.color}>{stat.icon}</span>
          <div>
            <div className={`text-2xl font-black ${stat.color}`}>{stat.count}</div>
            <div className="text-xs text-white/50 font-mono tracking-wide">{stat.platform}</div>
          </div>
        </motion.a>
      ))}
    </motion.div>
